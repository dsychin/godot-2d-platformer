...
class TestPlatformEdge:
	...
	var Scene: PackedScene = load("res://test/resource/platform_edge.tscn")
	var _scene: Node = null
	var _player: CharacterBody2D = null
	var _sender = InputSender.new(Input)
	...
	func test_player_should_be_able_to_jump_after_leaving_floor():
		_player.get_node("CoyoteTimer").wait_time = 1.0
		await _sender.action_down("ui_right").hold_for(.3).idle
		assert_false(_player.is_on_floor(), "player should have left the platform")
		await _sender.action_down("ui_accept").wait_frames(1).idle

		assert_lt(_player.velocity.y, 0.0, "player should have jumped")